...
assert_true(str_contains($javascript, 'withTimeout(OneSignal.User.PushSubscription.optIn(), 8000)'), 'push enablement and token recovery do not hang forever when the browser push API stalls');
...
$debugView = file_get_contents(dirname(__DIR__) . '/app/Views/admin/debug.php');
assert_true(str_contains(file_get_contents(dirname(__DIR__) . '/public/index.php'), "'/admin/debug'"), 'administrators have a dedicated push diagnostics route');
assert_true(str_contains($adminController, 'public function debug(): void'), 'the admin controller assembles safe server diagnostics');
...
assert_true(str_contains($javascript, 'automatische herregistratie wordt uitgevoerd'), 'push diagnostics report token recovery attempts');
assert_true(str_contains($javascript, 'return waitFor(() => OneSignal.User.externalId === oneSignalUser'), 'OneSignal initialization waits until the linked external user ID is exposed');
assert_true(substr_count($javascript, 'window.OneSignalDeferred.push(') === 1, 'OneSignal initialization has a single deferred SDK callback');
assert_true(str_contains($javascript, 'const runtime = await oneSignalRuntimeReady'), 'push diagnostics wait for completed OneSignal initialization before reading runtime state');
