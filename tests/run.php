...
assert_true(str_contains($memberProfilePage, 'name="notification_preference"'), 'the profile page exposes notification preferences');
assert_true(str_contains($memberProfilePage, 'Alleen vervallen taken') && str_contains($memberProfilePage, 'Alle notificaties uit'), 'the profile distinguishes expired-task-only notifications from disabling everything');
...
$users->setNotificationPreference((int) $member['id'], 'expired_only');
assert_true($subscriptions->idsForUsers([(int) $member['id']]) === [], 'general notifications exclude users who only want expired-task alerts');
assert_true($subscriptions->idsForUsers([(int) $member['id']], 'expired') === ['22222222-2222-4222-8222-222222222222'], 'expired-task notifications still reach users with the expired-only preference');
$users->setNotificationPreference((int) $member['id'], 'none');
assert_true($subscriptions->idsForUsers([(int) $member['id']], 'expired') === [], 'disabling all notifications also suppresses expired-task alerts');
$users->setNotificationPreference((int) $member['id'], 'all');
...
assert_true(str_contains($routes, "'/settings/notifications'"), 'signed-in users can save their notification preference');
